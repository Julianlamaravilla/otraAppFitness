...
const suggest_1 = require("../nutrition/suggest");
describe('Nutrition Suggestion Logic', () => {
    it('should calculate calories correctly for weight gain', async () => {
        ...
        const response = await (0, suggest_1.suggest)(event);
    ...
    it('should filter out restricted meals (e.g. Vegan)', async () => {
        ...
        const hasSalmon = body.suggestions.some((m) => m.name.includes('Salmon'));
        const hasChicken = body.suggestions.some((m) => m.name.includes('Chicken'));
    ...
    it('should filter out specific ingredients like Peanut', async () => {
        ...
        const response = await (0, suggest_1.suggest)(event);
        const body = JSON.parse(response.body);
        const hasPeanut = body.suggestions.some((m) => m.name.includes('Peanut'));
