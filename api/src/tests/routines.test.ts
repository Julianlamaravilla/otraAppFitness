import { generate } from '../routines/generate';
...
describe('Routine Generation Logic', () => {
  it('should filter out exercises that match injuries', async () => {
    ...
    const response = await generate(event);
    const body = JSON.parse(response.body);
    
    // Squats and Running target 'knee'. They should be absent.
    const hasSquats = body.routine.some((ex: any) => ex.name === 'Squats');
    const hasRunning = body.routine.some((ex: any) => ex.name === 'Running');
